class php::params {
  case $::osfamily {
    'Debian': {
      $php_package_name = 'php5'
      $php_apc_package_name = 'php-apc'
      $common_package_name = 'php5-common'
      $cli_package_name = 'php5-cli'
      $php_conf_dir = '/etc/php5/conf.d'
      $fpm_package_name = 'php5-fpm'
      $fpm_service_name = 'php5-fpm'
      $fpm_pool_dir = '/etc/php5/fpm/pool.d'
      $fpm_conf_dir = '/etc/php5/fpm'
      $fpm_error_log = '/var/log/php5-fpm.log'
      $fpm_pid = '/var/run/php5-fpm.pid'
      $httpd_package_name = 'apache2'
      $httpd_service_name = 'apache2'
      $httpd_conf_dir = '/etc/apache2/conf.d'
    }

    default: {
      $php_package_name = 'php55'
      $php_apc_package_name = 'php55-pecl-apc'
      $common_package_name = 'php55-php-common'
      $cli_package_name = 'php55-php-cli'
      $php_conf_dir = '/opt/remi/php55/root/etc/php.d'
      $fpm_package_name = 'php55-php-fpm'
      $fpm_service_name = 'php55-php-fpm'
      $fpm_pool_dir = '/opt/remi/php55/root/etc/php-fpm.d'
      $fpm_conf_dir = '/opt/remi/php55/root/etc/'
      $fpm_error_log = '/opt/remi/php55/root/var/log/php-fpm/error.log'
      $fpm_pid = '/opt/remi/php55/root/var/run/php-fpm/php-fpm.pid'
      $httpd_package_name = 'httpd'
      $httpd_service_name = 'httpd'
      $httpd_conf_dir = '/etc/httpd/conf.d'
    }
  }
}
